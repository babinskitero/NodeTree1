...
using static NodeTree.NodeTree<T>;
...
namespace NodeTree
{
	public abstract class Node
	{
		...
		public IEnumerable<NodeXmlSerializationAdapter> Children { get; internal set; }
	...
	{
		public IEnumerable<Node> Childrens { get; }
		public ManyChildrenNode(string name, params Node[] childrens) : base(name)
		{
			Childrens = childrens;
		}
	}
	...
	public class NodeDescriber : INodeDescriber
	{
		...
		{
			ITree<Node> tree = NodeTree<Node>.NewTree();
			foreach(NodeXmlSerializationAdapter child in node.Children)
			{
				tree.AddChild(child.Data);
			}

			var s = string.Empty;
			foreach(INode<Node> itemNode in tree.All.Nodes)
				s += new String('\t', itemNode.Depth) + itemNode + Environment.NewLine;
	...
	public class NodeWriter : INodeWriter
	{
		...
		{
			using(StreamWriter writer = File.CreateText(filePath + "newfile.txt"))
			{
